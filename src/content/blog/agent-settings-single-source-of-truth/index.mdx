...
<KeyTakeaways>
  <p>這篇說明如何讓 Claude 與 Codex 的 Agent 角色設定不必各寫一份：Claude 用一份 Markdown 同時服務 subagent 與主 session，Codex 則拆成 custom agent TOML 與 profile 兩條入口；作者把角色工作原則抽成 shared contract、角色名單收進 registry.yaml，兩邊 runtime 檔案只留各自必要的載入設定。</p>
</KeyTakeaways>
