...
<KeyTakeaways>
  <p>已經知道要叫哪個 Agent、用哪個帳號時，還需要讓 Skill 一題一題問嗎？我把 cmux 啟動流程抽成 summon script：輪詢選單搬到終端機，也能帶參數直接執行，Agent 名單則收進 registry 讓 script 與 Skill 共用。輪詢設計沒有被拿掉，只是移出對話、不再由 LLM 執行機械步驟。</p>
</KeyTakeaways>
