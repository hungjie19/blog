...
<KeyTakeaways>
  <p>Skill 是不是裝越多越好？我的 47 個 user Skills 光名稱與說明就約 20 KB，subagent 啟動時還會再付一次；用 /doctor 盤點後，我把每個 Skill 分到預載、按需觸發、共同 reference 與移除四個去處。診斷報告只讀得到單一帳號的 usage，不能照著低 usage 直接刪。</p>
</KeyTakeaways>
