...
<KeyTakeaways>
  <p>要怎麼讓 AI 直接讀到自己的 Obsidian 筆記？Obsidian MCP 是讓 AI 助理自行搜尋、讀取本機 vault 裡 Markdown 全文的橋接方式，不用先指定資料夾和關鍵字；我選的是不必開 App、不必裝 plugin 的 MCPVault，而它與 OpenMemory、對話摘要是分工，不是三選一。</p>
</KeyTakeaways>
