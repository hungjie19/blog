...
<KeyTakeaways>
  <p>無法使用 Claude 內建 Remote Control 時，還有沒有別的方式從手機控制本機 agent？Oysterun 是一套自帶手機對話、檔案瀏覽與通知的本機 AI Host，靠 Tailscale 讓手機隨時連回 Mac；但它只能接續自己開出的 session，terminal 裡既有的對話接不回來。</p>
</KeyTakeaways>
