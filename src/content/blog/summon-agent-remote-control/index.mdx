...
<KeyTakeaways>
  <p>手機遠端控制只能接手已經在跑的工作嗎？把 --remote-control 直接寫進 summon script 的啟動指令，新 Agent 一出生就能從手機接手，不必事後補送 /rc；建立 workspace 時要加 --focus true，否則 cmux 不會真的送出啟動命令。這條路只對手機 App 登入的 personal 帳號成立。</p>
</KeyTakeaways>
