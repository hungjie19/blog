...
<KeyTakeaways>
  <p>生圖 Prompt 有標準格式嗎？沒有唯一標準，但我從踩坑反推出 Intent → Style → Subject → Scene → Hard Constraints 五段骨架，並對照 OpenAI、Google、Midjourney 的官方建議；實務上要寫效果而不是數量，比較版本時一次只改一個變數。這是我的工作筆記式模板，不是官方規格。</p>
</KeyTakeaways>
