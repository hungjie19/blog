...
<KeyTakeaways>
  <p>OG image 值不值得自己寫？記錄從 1200×630 與 safe zone 的規格研究、用 og-template.html 討論版面，到翻成 Canvas API 在 Astro build time 自動生成，中間踩到 avatar 裁切、中文折行與 CJK 字體三個坑；不需要客製版面的話，直接用 astro-og-canvas 會快很多。</p>
</KeyTakeaways>
