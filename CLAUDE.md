...
- `series` + `seriesOrder`：選填，有明確閱讀順序的系列填相同 `series` ID（英文 kebab-case）與 `seriesOrder` 排序數字。**開新系列要在 `src/lib/series.ts` 的 `SERIES` 註冊 ID → 顯示名稱**（未註冊 fallback 顯示 ID）。文章底部自動出現系列 box，列出全系列（按 `seriesOrder` 排序、目前篇高亮），與發文時間無關，系列可以之後再補篇
- `seriesType: collection`：選填。用於同一主題下、但**沒有建議閱讀順序**的文章集合；同樣填 `series`，但不填 `seriesOrder`。文章底部會顯示無編號、帶圓點的相關文章清單，例如 Remote Control 方法地圖。未填時預設為有順序的 `series`。
